...
with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
    ...
    for sheet_name in xlsx.sheet_names:
        ...
        #df = df.drop(index=range(43, 48))  #not necessary for fall 23 
        ...
        for participant_id in df.columns:
            ...
            results.setdefault('First 5 Consecutive Day Mean', []).append(first_valid_days.mean().__round__(2) if first_valid_days is not None else "Not sufficient")
            results.setdefault('First 5 Consecutive Day Std Dev', []).append(first_valid_days.std().__round__(2) if first_valid_days is not None else "Not sufficient")
            results.setdefault('Last Consecutive 5 day mean', []).append(last_valid_days.mean().__round__(2) if last_valid_days is not None else "Not sufficient")
            results.setdefault('Last Consecutive 5 day Std Dev', []).append(last_valid_days.std().__round__(2) if last_valid_days is not None else "Not sufficient")

        # Create a DataFrame and write to the corresponding sheet
        new_df = pd.DataFrame(results)
        new_df = new_df.merge(tier_info, on='Participant ID', how='left')
        # Optionally move 'Intervention Group' column right after 'Participant ID'
